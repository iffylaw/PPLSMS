...
taylor.diagram<-function(ref, model, add = FALSE, col = "red", pch = 19, pos.cor = TRUE,lim=lim,
    ...
    #R <- cor(ref, model, use = "pairwise")   # use only the valid pair of data
    #R <- cor(ref, model, use = "pairwise.complete.obs") 
    R <- cor(ref, model, use = "na.or.complete") 

    sd.r <- sd(ref,na.rm=TRUE)      
    sd.f <- sd(model,na.rm=TRUE)
